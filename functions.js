...
function setupAutocomplete(inputId, modalId, hiddenInputId, searchUrl) {
                ...
                source: function (request, response) {
                    $.ajax({
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                            // Error silencioso en producción o podrías manejarlo de otra forma
                        }
                    });
                },
                minLength: 2,
...
if (typeof timeline !== 'undefined' && typeof items !== 'undefined') {
...
function topatient(newpid, enc) {
    top.restoreSession();
    if (enc > 0) {
        top.RTop.location = "/interface/patient_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid) + "&set_encounterid=" + encodeURIComponent(enc);
    } else {
        top.RTop.location = "/interface/patient_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid);
    }
}
//
