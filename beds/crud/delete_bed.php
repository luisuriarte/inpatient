...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $datetimeDelete = date('Y-m-d H:i:s');
    $active = 0;

// Preparar la consulta para actualizar la cama en la tabla beds
$query = "UPDATE beds 
          SET active = 0, operation = ?, user_modif = ?, datetime_modif = ?
          WHERE id = ?";
...
try {
    ...
    sqlStatement($query, [$operation, $userFullName, $datetimeDelete, $bedId]);
    ...
    if ($recentBedPatient) {
        ...
        $queryUpdate = "UPDATE beds_patients 
                        SET user_modif = ?, datetime_modif = ?, operation=?, active=? 
        ...
        sqlStatement($queryUpdate, [$userFullName, $datetimeDelete, 'Bed Deleted', $active, $bedPatientId, $bedId]);
    }
