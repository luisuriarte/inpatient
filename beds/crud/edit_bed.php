...
$bedId = isset($_GET['bed_id']) ? intval($_GET['bed_id']) : 0;
$bedName = isset($_GET['bed_name']) ? htmlspecialchars($_GET['bed_name']) : 0;

// Si el ID de la cama no es válido, redirigir o mostrar un error
if ($bedId == 0) {
    echo "Cama no válida.";
    exit();
}

// Obtener los detalles de la cama desde la base de datos
$query = "SELECT * FROM beds WHERE id = ?";
$bedResult = sqlStatement($query, [$bedId]);
$bed = sqlFetchArray($bedResult);
...
// Obtener las opciones para los dropdowns de tipo y estado de cama
...
<head>
    ...
    <title><?php echo xlt('Edit Bed'); ?></title>
            ...
            <div class="modal-content">
                <form method="post" action="update_bed.php">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editBedModalLabel"><?php echo xlt('Edit Bed'); ?></h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" name="bed_id" value="<?php echo htmlspecialchars($bed['id']); ?>">
                        <input type="hidden" name="uuid" value="<?php echo htmlspecialchars($bed['uuid']); ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="bedName" name="bed_name" value="<?php echo htmlspecialchars($bed['bed_name']); ?>" required>
                        </div>
                            ...
                            <select class="form-select" id="bedType" name="bed_type" required>
                                ...
                                if (sqlNumRows($resultBedType) > 0) {
                                    while ($row = sqlFetchArray($resultBedType)) {
                                        $selected = ($row['title'] == $bed['bed_type']) ? 'selected' : '';
                                        echo '<option value="' . xlt($row['title']) . '" ' . $selected . '>' . xlt($row['title']) . '</option>';
                                    }
                                }
                                ?>
                            </select>
                            ...
                            <select class="form-select" id="bedStatus" name="bed_status" required>
                                ...
                                if (sqlNumRows($resultBedStatus) > 0) {
                                    while ($row = sqlFetchArray($resultBedStatus)) {
                                        $selected = ($row['title'] == $bed['bed_status']) ? 'selected' : '';
                                        echo '<option value="' . xlt($row['title']) . '" ' . $selected . '>' . xlt($row['title']) . '</option>';
                                    }
                                }
                                ?>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="bedObs" name="obs" rows="2"><?php echo htmlspecialchars($bed['obs']); ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="bedActive" class="form-label"><?php echo xlt('Active'); ?>:</label>
                            <label class="custom-checkbox">
                                <input type="checkbox" id="bedActive" name="active" value="1" <?php echo ($bed['active'] == 1) ? 'checked' : ''; ?>>
